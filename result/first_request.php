...
$query = "SELECT DISTINCT s.cost - p.contributed_money, cl.fio FROM payment p INNER JOIN contracts c ON p.contract_id = c.id_contract INNER JOIN client cl ON c.client_id = cl.id_client INNER JOIN subscription_fee s ON s.id_subscription = c.subscription_id WHERE s.cost - p.contributed_money > 0;";
                ...
                if ($result = mysqli_query($connection, $query)) {
                    if (mysqli_num_rows($result) > 0) {
                        echo "<table class='table table-bordered table-striped'>
                                ...
                                <tr>
                                <th>ПІБ</th>
                                <th>Сума заборгованості</th>
                                </tr>
                               </thead>
                               <tbody>";
                        while ($row = $result->fetch_assoc()) {
                            $temp = $row['s.cost - p.contributed_money'];
                            echo "<tr>
                                    <td>$row[fio]</td>
                                    <td>$temp</td>
                                    </tr>";
