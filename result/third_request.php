...
<head>
    ...
    <title>Редагування тарифів</title>
</head>
<body>
<?php
session_start();
        ...
        <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            ...
            <div class="navbar-nav">
                <a class="nav-link active" aria-current="page"
                   href="../subscription_fee/subscription_fee.php">Підписка</a>
            </div>
            <div class="navbar-nav">
...
<form method="post">
    <div class="container">
        <div class="form-floating" style="margin-bottom: 5%">
            <input type="text" class="form-control" id="floatingInput" placeholder="name_tariff" name="address">
            <label for="floatingInput">Введіть адресу</label>
            <button class="w-100 btn btn-lg btn-primary" type="submit"
                    style="margin-top: 2%">Перевірити
            </button>
        </div>
        <div class="row">
            <div class="col-md-12">
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    if (empty($address)){
                        ...
                        $query = "SELECT p.contributed_money, cl.fio, tr.tariff_name FROM payment p INNER JOIN contracts cn ON p.contract_id = cn.id_contract INNER JOIN client cl ON cn.client_id = cl.id_client INNER JOIN tariffs tr ON cl.tariff_id = tr.id_tariff WHERE MONTH(NOW()) = MONTH(p.payment_date) AND cl.address = '$address'";
                        $result = mysqli_query($connection, $query) or die("Не вдалося встановити з'єднання");
                    if (mysqli_num_rows($result) > 0) {
                        echo "<table class='table table-bordered table-striped'>
                                <thead>
                                <tr>
                                <th>Внесена сума за місяць</th>
                                <th>ПІБ</th>
                                <th>Назва тарифу</th>
                                </tr>
                               </thead>
