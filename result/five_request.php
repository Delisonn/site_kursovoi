...
$query = "SELECT client.fio, TIMESTAMPDIFF(MONTH, CONCAT(YEAR(now()),'-01-01'), now())-count(payment.id_payment) as not_pay FROM contracts INNER JOIN client ON contracts.client_id=client.id_client INNER JOIN payment ON contracts.id_contract=payment.contract_id GROUP by fio HAVING not_pay>0";
